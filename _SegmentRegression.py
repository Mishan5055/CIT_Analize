...
from TomographyAnalysisBasicFunction import EXPERIMENT
...
def _3D_SegmentRegression(
    exps: list[EXPERIMENT], l_Ts, l_Xs, l_Hs, s_Ts, s_Xs, s_Hs, dT, name, lam=1.0e3
):
    """_summary_
    ...
    for ih in range(n_H):
        M, A, B, lX = SegmentRegression(
            ...
            dT,
            lam=lam,
            Tbound=[min(l_Ts) - 0.0001, max(l_Ts) + 0.0001],
        )
        Ms.append(M)
        As.append(A)
        Bs.append(B)
    ...
    for ih in range(n_H):
        ax01.plot(a2t, As[ih], marker="x", c=colors[ih], label="{i}".format(i=ih))
        ax01.axhline(y=vel_average[ih], linestyle="dotted", c=colors[ih], linewidth=3)
    ...
    for ih in range(n_H):
        ax02.plot(a2xs[ih], As[ih], marker="x", c=colors[ih], label="{i}".format(i=ih))
    ...
    os.makedirs(
        "D:/tid/{c}/{y4:04d}/{d:03d}/{code}/".format(
            c=exps[0].c, y4=exps[0].y, d=exps[0].d, code=exps[0].code
        ),
        exist_ok=True,
    )
    fig.savefig(
        "D:/tid/{c}/{y4:04d}/{d:03d}/{code}/{n}.png".format(
...
def SegmentRegression(l_Ts, l_Xs, dT, lam=1.0e3, Tbound=[np.nan, np.nan]):
    ...
    if len(l_Ts) < 2 or len(l_Xs) < 2:
        return (
            M,
            np.full((M), np.nan, dtype=float),
            np.full((0), np.nan, dtype=float),
            a1t,
        )
    n_t = len(a1t) - 1
...
if __name__ == "__main__":
    ...
    _3D_SegmentRegression(l_Ts, l_Xs, l_Hs, s_Ts, s_Xs, s_Hs, 1.0, lam=1.0e3)
